<template>
  <div class="map-container">
    ...
    <MapMeasure @polyline="measurePolyline" @polygon="measurePolygon" @end="drawOff"></MapMeasure>
  </div>
</template>

<script>
import MapMeasure from "@/components/MapMeasure.vue";

export default {
  name: "map-point",
  components: { MapMeasure },
  data() {
    return {
      map: null,
  ...
  methods: {
    ...
    measurePolygon() {
      ...
      this.map.on("dblclick", () => {
        ...
        let polygon = this.$utils.map.createPolygon(
          this.map,
          this.tempGp.polygonNode,
          finalPolygonOpts
        );
